...
15. Append Chinese power analysis and error analysis sections in `tasklist.txt`.
16. For power analysis, parse component datasheets (when available), identify power topology from netlist, and apply layout-aware penalties from Gerber net span data.
17. In `tasklist.txt`, include Chinese stage-level power breakdown (Vin->Vout, Iout, Pout, Ploss, efficiency, parameter source).
18. In `tasklist.txt`, append a netlist-driven section `#基于网表的PCB布局建议与注意事项` with concrete layout guidance and cautions.
19. Read generated analysis bundle and tasklist.
20. Return conclusion first in Chinese.
